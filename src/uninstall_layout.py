...
class UninstallLayout(tgo.LinearLayout):
    ...
    def on_uninstall(self, e, v):
        if len(self.selected) == 0:
            return
        self.a.set_active(self.a.uninstall_progress)
        self.a.uninstall_progress.set_pm(self.pm, self.selected)
        self.selected = []
        for b in self.boxes:
            b.setchecked(False)
    ...
    def set_pm(self, pm: PM):
        self.pm = pm
        self.pgklist = []
        self.pkgrequest = pm.list_installed()
    ...
    def __init__(self, a: PackageActivity):
        ...
        self.installbutton = tgo.Button(self.a, "Uninstall", self.bar)
        self.installbutton.on_click = self.on_uninstall
